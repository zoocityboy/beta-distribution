...
@router.get(
    "/profile/udid_profile.mobileconfig",
    response_class=HTMLResponse,
)
async def get_download_profile(
    request: Request,
) -> HTMLResponse:

    content = templates.TemplateResponse(
        request=request,
        name="udid_profile.mobileconfig",
        media_type="application/xml",
        context={
            "uuid": str(uuid4()),
            "url": get_absolute_url("/profile/udid_profile.mobileconfig"),
        },
    ).body
    ...
    return Response(
        content=content,
        media_type="application/octet-stream",
        headers={"Content-Disposition": f"attachment; filename=udid_profile.mobileconfig"},
    )
